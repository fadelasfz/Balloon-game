...
class C(BaseConstants):
    NAME_IN_URL = 'balloon_experiment'
...
class Balloon(Page):
    ...
    def vars_for_template(player: Player):
        if 'round_start_time' not in player.participant.vars:
            ...
            player.participant.vars['current_time_limit'] = 1200
    ...
    def live_method(player, data):
        ...
        return {player.id_in_group:dict(
                    nb_failed_attempts=player.nb_failed_attempts,
                    solution_to_find=player.solution_to_find,
                    found_solution=player.found_solution,
...
page_sequence = [PracticeBalloon, PracticeFeedback, Balloon, CombinedResults]
# page_sequence = [BasicDemographics, PerspectiveTaking, KnowBefore, GroupGame, KnowAfter, Balloon, CombinedResults]
# page_sequence = [Introduction, Balloon, CombinedResults, EndGame]
# page_sequence = [Introduction, Balloon, CombinedResults]